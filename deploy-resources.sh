...
AZURE_OPENAI_ENDPOINT=`az cognitiveservices account show --name ${AOAI_NAME} --resource-group ${AZURE_RESOURCE_GROUP} --query "properties.endpoints" | jq '."OpenAI Language Model Instance API"'`
AZURE_OPENAI_ENDPOINT=$(echo $AZURE_OPENAI_ENDPOINT | sed 's/"//g')
AZURE_OPENAI_ENDPOINT=$(echo $AZURE_OPENAI_ENDPOINT | sed 's/\/$//')
...
STORAGE_ACCOUNT_CONNECTION_STRING=`az storage account show-connection-string --name ${STORAGE_ACCOUNT_NAME} --resource-group ${AZURE_RESOURCE_GROUP} | jq '."connectionString"'`
...
echo "========================================"
echo "Build and Push (Create Annotation Job)"
echo "========================================"
cd ./app/annotation
...
echo "========================================"
echo "Creating Container Apps Job (Create Annotation Job)"
echo "========================================"
cd ../search
ACR_USER=`az acr credential show --name ${ACR_NAME} --resource-group ${AZURE_RESOURCE_GROUP} --query username -o tsv`
ACR_PASS=`az acr credential show --name ${ACR_NAME} --resource-group ${AZURE_RESOURCE_GROUP} --query "passwords[0].value" -o tsv`
